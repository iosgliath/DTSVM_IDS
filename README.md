...
Genetic Algorithm is used at each node to try find the best solution possible to this combination problem (distribute all node's classes into two subsets Xp and Xn). Binary SVM is then trained on this subdataset. If Xp or Xn are only one class, we got a leaf node, else, an inner node.
