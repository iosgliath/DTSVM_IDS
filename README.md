...
The intuition is that we want to separate our dataset at each nodes with the right combination of classes (the one that maximise the euclidan distance between the two combinations of classes) => it gves more "room" for our binary SVM to draw support vectors. 
